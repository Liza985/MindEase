...
export const updateById = async (req, res) => {
    try {
        ...
        const updatedContent = await Activity.findByIdAndUpdate(id, req.body, { new: true });
        if (!updatedContent) {
            return Response(res, 404, false, message.contentNotFound); 
        }
        Response(res, 200, true, message.contentUpdatedSuccessfully, updatedContent); 
...
export const getContentByCategory = async (req, res) => {
    try {
        ...
        const content = await Activity.find({ category: category }).populate("author", "_id username");
