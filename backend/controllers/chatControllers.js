...
export const getChatsByVolunteer = async (req, res) => {
  try {
    ...
    if (Chat) {
      ...
      // Using in-memory storage
      const volunteerChats = Array.from(activeChats.values())
        .filter(chat => chat.volunteerId === volunteerId);
    ...
    console.error(`Error fetching chats for volunteer ${req.params.volunteerId}:`, error);
    return res.status(500).json({
      success: false,
